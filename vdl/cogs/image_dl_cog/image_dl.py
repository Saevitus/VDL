...
class image_dl_cog(commands.Cog, img_dl_mixin):
    ...
    async def cmd(self, ctx: disnake.ApplicationCommandInteraction, url: str, custom_file_name: str = ""):
        await ctx.response.defer(with_message=False)
    
        if len(custom_file_name) <= 0: #if supplied a custom file name, skip creating a randomized one
