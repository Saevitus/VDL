...
class media_dl_cog(commands.Cog, yt_dl_mixin, tt_dl_mixin):
    ...
    async def download_media(self, ctx, url: str, custom_file_name: str = "", audio_only: bool = False):  
        '''
        Main media download function, it takes the discord bot context, 
        url and optional paramaters custom_file_name and audio_only
        '''

        if len(str(custom_file_name)) <= 0: #if supplied a custom file name, skip creating a randomized one
                custom_file_name = ''.join(random.choices(string.ascii_letters + string.digits, k=8))
        ...
        file_details = [ctx, url, custom_file_name, audio_only]
        ...
        if "tiktok.com" in url:
                ...
                #print("test " + file_details[1])
                self.media = await self.yt_dl(file_details) # if not tiktok, download using yt-dlp
    ...
    async def on_message(self, ctx):
        ...
        if ctx.guild:
            ...
            if not (id == 1081923842487877702 or id == 1164523892073836574 or
                "dl-channel" in name or "owna" in name):
        ...
        if ctx.channel.id == 1081923842487877702:
            ...
            if self.media[0].filename == "FSTB.png":
                await ctx.channel.send(content = "File size too big.", files = self.media, delete_after = 3)
    ...
    async def cmd(self, ctx: disnake.ApplicationCommandInteraction, url: str, custom_file_name: str = "", audio_only: bool = False):
        '''
        ...
        if ctx.guild:
            ...
            if not (id == 1081923842487877702 or id == 1164523892073836574 or
                "dl-channel" in name or "owna" in name):
                await ctx.followup.send(content = "Wrong channel, please use the correct channels for the bot", delete_after = 3)
                return
        ...
        '''
        if self.media[0].filename == "FSTB.png":
            await ctx.followup.send(content = "File size too big.", files = self.media, delete_after = 3)
            for fn in os.listdir("vdl/media/"):
                os.remove("vdl/media/" + fn)
            return
        ...
        if(len(self.media) > 9):
            l = utils.split(self.media, 10) # split into chunks of 10 and store in a new list
            for i in l:
